...
from recode.frame import Actor, EpisodicCritic, Reward, DenseCritic
from recode.frame import score_function as Score_Func
from recode.utils.data_loader import DataGenerator
from recode.utils.graph_analysis import get_graph_from_order, pruning_by_coef
from recode.utils.graph_analysis import pruning_by_coef_2nd
#from recode.utils.analyze_utils import get_graph_from_order, pruning_by_coef
#from recode.utils.analyze_utils import pruning_by_coef_2nd
...
class CORL(BaseLearner):
    ...
    def __init__(self, batch_size=64, input_dim=100, embed_dim=256,####################################################
                 ...
                 decoder_name='lstm',
                 reward_mode='episodic',       #dense & episodic
    ...
    def learn(self, data, columns=None,**kwargs) -> None: #############################################################
        ...
        #causal_matrix = self._rl_search(X, _adj)##############################################################################
    ...
    def _rl_search(self, X) -> torch.Tensor:###########################################################################
        ...
        for i in tqdm(range(1, self.iteration + 1)):
            ...
            encoder_output = actor.encode(input = input_batch.float()) #input_batch.float()
