...
utils.install_packages(StrVector(['SID']))
...
def pruning_by_coef(graph_batch, X, th=0.3,if_normal=True):  # if_normal=True,th=0.15,if_normal=False,th=0.3
    n, d = X.shape
...
def pruning_by_coef_2nd(graph_batch, X, th=0.3):
    d = len(graph_batch)
    reg = LinearRegression()
...
def _pns(model_adj, all_samples, num_neighbors, thresh=0.75):
    ...
    for node in tqdm(range(num_nodes), desc='Preliminary neighborhood selection'):
        ...
        model_adj.iloc[:, node] *= mask_selected
    return model_adj
