...
'''
    def process_coefficients(reg_coeff,if_normal=True):
    # 使用标准化器对系数进行标准化
    scaler = StandardScaler()
    reg_coeff = scaler.fit_transform(reg_coeff.reshape(-1, 1))
    return reg_coeff
    '''
def process_coefficients(reg_coeff, if_normal=True):
    if if_normal:
...
def pruning_by_coef(graph_batch, X, thresh=0.4) -> np.ndarray:# thresh=0.30
    """
    ...
    for i in range(d):
        ...
        reg_coeff = reg.coef_
        # 调用系数处理函数
        #reg_coeff = process_coefficients(reg_coeff, if_normal=True)
